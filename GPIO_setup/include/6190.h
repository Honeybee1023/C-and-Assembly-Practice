...
void pinSetup(int pin_num, int mode){
    ...
    */
    ...
    if (mode == GPIO_INPUT){
        ...
        *io_mux_gpio_addr = *io_mux_gpio_addr | (0b11 << 8);
    } else if (mode == GPIO_OUTPUT){
        // Configure GPIO pin pin_num as output.
        int *gpio_enable_addr = (int*) GPIO_ENABLE_ADDR;
        *gpio_enable_addr = *gpio_enable_addr | (0b1 << pin_num);
    }
}
...
void pinWrite(int pin_num, int value){
    ...
    */
    ...
    if (value == 0){
        *output_val = *output_val & ~(0b1 << pin_num);
    }
    else{
        *output_val = *output_val | (0b1 << pin_num);
    }
...
int pinRead(int pin_num){
    /* Function to read an input GPIO pin (pin_num)
    Arguments:
        pin_num: GPIO pin number (0 - 31)
    */

    //////////////////////////////////////////
    // TODO: Complete pinRead() function.  //
    //////////////////////////////////////////

    int *input_val = (int*) GPIO_IN_ADDR;

    if ((*input_val & (1 << pin_num)) != 0){
        return 1;
    }
    else{
        return 0;
